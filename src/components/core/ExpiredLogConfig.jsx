...
const ExpiredLogConfig = ({ refetchKey = "cslogs" }) => {
  ...
  const { mutate: updateExpiry } = useMutation({
    ...
    onError: () => {
      toast.error("Gagal mengubah konfigurasi default expiry", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: true,
        closeOnClick: true,
      ...
      <div className="flex flex-col md:flex-row items-center justify-between w-full mb-4">
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          {daysLoading ? (
            ...
            <p className="text-4xl font-bold text-[#F87171] tracking-tight">
              {defaultDays}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center flex-1 justify-end w-full md:w-auto">
          ...
          <input
            ...
            className="border border-gray-300 px-3 py-1 rounded w-24 text-gray-700 
                       focus:border-[#F87171] focus:ring-1 focus:ring-[#F87171] 
                       focus:outline-none transition-all"
          />
          <button
            onClick={() => updateExpiry(Number(newExpiry))}
            disabled={!newExpiry}
            className="ml-4 bg-[#F87171] hover:bg-[#ef4444] text-white px-4 py-1 
                       rounded font-medium disabled:opacity-50 
                       disabled:hover:bg-[#F87171] transition-colors"
          >
            Simpan
          </button>
        </div>
